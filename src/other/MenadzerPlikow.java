...
public class MenadzerPlikow {
    public void zapisz(Swiat swiat, File plik){
        try {
            ...
            out.write(swiat.getNrTury() + " " + swiat.getHeight() + " " + swiat.getHeight()  + " " + typStr+ "\n");

            for(Organizm org : swiat.getOrganizmy()){
                out.write(org.toString() + " " + org.getWiek() + " " + org.getPolozenie().getY() + " " +
    ...
    private Organizm wczytajOrganizm(String line) throws Exception {
        ...
        if(nowy != null){
            ...
            nowy.setPolozenie(new Point(Integer.parseInt(atryb[2]), Integer.parseInt(atryb[3])));
